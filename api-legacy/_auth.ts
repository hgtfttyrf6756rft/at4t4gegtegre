 ...
 * v1.1: Added requireSubscription for Pro user gating
 */
...
/**
 * Require subscription (Pro user check).
 * Must be called after requireAuth.
 */
export async function requireSubscription(request: Request): Promise<void | Response> {
  ...
  try {
    ...
    if (!userData?.subscribed) {
      return jsonError('Pro subscription required', 403);
    }

    return; // User is subscribed, allow access
  } catch (error: any) {
    console.error('[api/_auth] Subscription check failed:', error);
