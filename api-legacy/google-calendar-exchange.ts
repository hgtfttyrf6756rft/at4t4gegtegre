...
const getRedirectUri = (request: Request): string => {
    ...
    // If explicit generic URI is set, we might need a specific one for calendar if the backend distinguishes,
    // but for now, we'll try to construct it from the request origin + specific path
    // just like the frontend expects.
    const url = new URL(request.url);
    return `${url.origin}/google-calendar/callback`;
};
...
export default {
    async fetch(request: Request): Promise<Response> {
        ...
        // STOPPING CONFLICT: Save to `googleCalendar` integration path, NOT `googleDrive`
        const ref = db.doc(`users/${authResult.uid}/integrations/googleCalendar`);
